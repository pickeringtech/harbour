...
use PickeringTech\Harbour\Integrations\Worktrunk\WorktrunkInstallation;
...
final class InstallCommand extends WorkspaceCommand
{
    protected $signature = 'workspace:install
        ...
        {--worktree-hooks= : Worktree lifecycle integration: none, orca, or worktrunk}
    ...
    public function handle(
        ...
        OrcaIntegration $orca,
        WorktrunkIntegration $worktrunk,
    ): int {
        $json = (bool) $this->option('json');

        return $this->executeSafely($json, function () use ($installer, $detector, $starter, $services, $preflight, $dependencies, $runtime, $application, $orca, $worktrunk, $json): int {
            ...
            $worktreeHooks = $this->worktreeHooks($json);
            $orcaInstallation = in_array('orca', $worktreeHooks, true) ? $orca->prepare((bool) $this->option('reconfigure')) : null;
            $worktrunkInstallation = in_array('worktrunk', $worktreeHooks, true) ? $worktrunk->prepare() : null;
            ...
            $dependenciesApproved = $this->installDependencies($selection, $preflight, $dependencies, $json, $plan->start, $worktreeHooks);
            if ($json) {
                $this->assertReady($preflight, $selection, $plan->start, $dependenciesApproved, $worktreeHooks);
            } else {
                $this->components->task('Checking selected stack requirements', fn () => $this->assertReady($preflight, $selection, $plan->start, $dependenciesApproved, $worktreeHooks));
            }
            $result = $installer->install($discovery, (bool) $this->option('reconfigure'));
            if ($orcaInstallation instanceof OrcaInstallation) {
                $orca->install($orcaInstallation);
                $result = $result->withProjectFile(OrcaIntegration::CONFIGURATION_PATH, $orcaInstallation->change, 'orca');
            }
            if ($worktrunkInstallation instanceof WorktrunkInstallation) {
                $worktrunk->install($worktrunkInstallation);
                $result = $result->withProjectFile(WorktrunkIntegration::CONFIGURATION_PATH, $worktrunkInstallation->change, 'worktrunk');
            }
            $startOutput = '';
            ...
            $this->components->twoColumnDetail('Worktree hooks', $worktreeHooks === [] ? 'none' : implode(', ', $worktreeHooks));
    ...
    /** @return list<string> */
    private function worktreeHooks(bool $json): array
    {
        $option = $this->stringOption('worktree-hooks');
        if ($option === null) {
            if ($json || ! $this->input->isInteractive()) {
                return [];
            }
            if (! confirm(
                label: 'Configure worktree lifecycle hooks?',
                default: false,
                hint: 'This writes only project-scoped configuration for integrations you select.',
            )) {
                return [];
            }

            $selected = multiselect(
                label: 'Which worktree integrations should Harbour configure?',
                options: [
                    'orca' => 'Orca IDE (requires verified blocking archive)',
                    'worktrunk' => 'Worktrunk (full lifecycle)',
                ],
                default: [],
                hint: 'Setup and teardown are blocking lifecycle hooks.',
            );

            return array_values(array_map(fn (mixed $choice): string => $this->choiceString($choice, 'worktree integration'), $selected));
        }
        ...
        if ($normalized === '') {
            throw new HarbourException(ErrorCode::InvalidInstallSelection, '--worktree-hooks must be none or a comma-separated integration list.');
        }

        $selected = array_values(array_unique(array_map('trim', explode(',', $normalized))));
        foreach ($selected as $integration) {
            if (! in_array($integration, ['orca', 'worktrunk'], true)) {
                throw new HarbourException(
                    ErrorCode::InvalidInstallSelection,
                    "Unsupported worktree lifecycle integration [{$integration}]. Choose one of: orca, worktrunk, none.",
                );
            }
        }
    ...
    /** @param list<string> $worktreeHooks */
    private function installDependencies(
        ...
        try {
            ...
            throw new HarbourException(
                ...
                [...$exception->context, 'retry_command' => $this->retryCommand($selection, $start, true, $worktreeHooks)],
                $exception,
            );
        }
    ...
    private function assertReady(
        ...
        try {
            ...
            $context['retry_command'] = $this->retryCommand($selection, $start, $dependenciesApproved, $worktreeHooks);
    ...
    /** @param list<string> $worktreeHooks */
    private function retryCommand(InstallationSelection $selection, bool $start, bool $installDependencies, array $worktreeHooks): string
    {
        ...
        $command .= ' --worktree-hooks='.($worktreeHooks === [] ? 'none' : implode(',', $worktreeHooks));
