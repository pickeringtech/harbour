...
final class ProjectInstallerTest extends TestCase
{
    ...
    public function test_it_prepares_an_idempotent_project_installation(): void
    {
        ...
        $reconfigured = $installer->install($selection, true);
        self::assertContains('.env.harbour', $reconfigured->unchanged);
        self::assertContains('config/harbour.php', $reconfigured->unchanged);
    }
    ...
    public function test_it_rejects_a_list_shaped_scripts_property_without_reformatting_the_manifest(): void
    {
        file_put_contents($this->workspace.'/composer.json', '{"name":"scripts","extra":{"scripts":{}},"scripts":[]}');
    ...
    public function test_unreadable_managed_gitignore_and_composer_files_fail_closed(): void
    {
        $cases = [
        ...
        foreach ($cases as [$relative, $contents]) {
            $workspace = $this->workspace.'/'.str_replace('.', '-', $relative);
            mkdir($workspace, 0700, true);
            file_put_contents($workspace.'/composer.json', '{"name":"acme/app"}');
            if (str_contains($relative, '/')) {
                mkdir($workspace.'/'.dirname($relative), 0700, true);
            }
            file_put_contents($workspace.'/'.$relative, $contents);
            chmod($workspace.'/'.$relative, 0000);
            try {
                (new ProjectInstaller($workspace))->install(self::selection());
                self::fail("Unreadable {$relative} must fail closed.");
            } catch (HarbourException $exception) {
                self::assertStringContainsString('Unable to read', $exception->getMessage());
            } finally {
                chmod($workspace.'/'.$relative, 0600);
            }
        }
    }

    public function test_it_rejects_a_directory_at_a_managed_file_path(): void
    {
        mkdir($this->workspace.'/.env.harbour');

        $this->expectException(HarbourException::class);
        $this->expectExceptionMessage('unsafe project path');
        (new ProjectInstaller($this->workspace))->install(self::selection());
    }
    ...
    public function test_it_accepts_whitespace_between_the_scripts_name_and_colon(): void
    {
        file_put_contents($this->workspace.'/composer.json', "{\n  \"scripts\" \n  : {\"test\":\"phpunit\"}\n}\n");

        (new ProjectInstaller($this->workspace))->install(self::selection());

        self::assertArrayHasKey('workspace:setup', $this->composerScripts());
    }
