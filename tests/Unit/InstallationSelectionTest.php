...
use PickeringTech\Harbour\Installation\InstallationResult;
...
use PickeringTech\Harbour\Installation\UninstallationResult;
...
final class InstallationSelectionTest extends TestCase
{
    ...
    public function test_direct_value_construction_enforces_provider_and_redis_client_invariants(): void
    {
        foreach ([
            ['shared-invalid', 'auto'],
            ['shared', 'invalid-client'],
        ] as [$provider, $client]) {
            try {
                new InstallationSelection('none', 'none', 'none', [], $provider, $client);
                self::fail('Expected direct construction to reject invalid values.');
            } catch (HarbourException $exception) {
                self::assertSame(ErrorCode::InvalidInstallSelection, $exception->errorCode);
            }
        }
    ...
    public function test_installation_result_classifies_integrated_project_file_changes(): void
    {
        $selection = InstallationSelection::fromOptions('sqlite', 'file', 'log', 'none');
        $result = new InstallationResult([], [], [], [], $selection);
        $result = $result->withProjectFile('.config/worktrunk.toml', 'created', 'worktrunk');
        $result = $result->withProjectFile('.config/worktrunk.toml', 'updated', 'worktrunk');
        $result = $result->withProjectFile('.config/worktrunk.toml', 'unchanged', 'worktrunk');

        self::assertSame(['.config/worktrunk.toml'], $result->created);
        self::assertSame(['.config/worktrunk.toml'], $result->updated);
        self::assertSame(['.config/worktrunk.toml'], $result->unchanged);
        self::assertSame(['worktrunk', 'worktrunk', 'worktrunk'], $result->worktreeHooks);

        $uninstallation = new UninstallationResult([], [], []);
        $uninstallation = $uninstallation->withProjectFile('.config/worktrunk.toml', 'removed');
        $uninstallation = $uninstallation->withProjectFile('.config/worktrunk.toml', 'retained');
        self::assertSame(['.config/worktrunk.toml'], $uninstallation->removed);
        self::assertSame(['.config/worktrunk.toml (not generated by Harbour or modified)'], $uninstallation->retained);
    }

    public function test_compose_renderer_rejects_a_shared_infrastructure_selection(): void
    {
        $this->expectException(\LogicException::class);
        (new InstallationComposeRenderer)->render(InstallationSelection::fromOptions('pgsql', 'none', 'none', 'none'));
    }

    public function test_runtime_resolution_can_use_the_actual_loaded_extension_set(): void
    {
        $selection = InstallationSelection::fromOptions('none', 'redis', 'log', 'none', 'shared', 'phpredis');
        $discovery = InstallationDiscovery::explicit($selection);
        $resolved = (new InstallationRuntimeResolver)->resolve($discovery);

        self::assertSame(extension_loaded('redis') ? 'phpredis' : 'predis', $resolved->selection->redisClient);
        self::assertSame('DB_PORT', (new InstallationServiceCatalog)->primaryPortVariable('mysql'));
    }
